...
ngOnInit(): void {
  ...
  this.idRelation = this.ODataP.idRelation;
            ...
            this.salesServ.CInsertPayments( this.paymentList, this.paymentForm.idCaja, this.paymentForm.idCustomer )
              .subscribe({
                next: async (resp: ResponseDB_CRUD) => {
                  ...
                  if( resp.status === 0 ){
                    this.printTicketServ.printTicket("Venta", this.idRelation, this.selectCajas);
                    this.dialogRef.close( this.idRelation );
                  }
...
fn_CerrarMDL(){
  this.dialogRef.close( this.ODataP.idRelation );
}
